                ...
                <div class="class-list col-md-8 col-md-offset-2">
                    <table class="table table-striped">
                        ...
                        <tr v-for="obj in resultList">
                            <td>{{obj.c_id}}</td>
                            <td>{{obj.c_name}}</td>
                            <td class="sub" data-toggle="modal" data-target="#myModal1" @click="choose">开始考试</td>
                        </tr>
                    </table>
        ...
        <div class="modal fade" id="myModal1" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        确认开始{{course}}考试?<span style="color: red">考试中途退出视为弃考</span>
                    </div>
                    <div class="modal-footer">
    ...
    var vm =new Vue({
        ...
        mounted:function () {
            ...
            $.getJSON('student/choice_list?callback=?',function (res) {
                _this.resultAll = res;
                $.getJSON('student/grades_list?callback=?',function (data){
                    var arr=_this.resultAll;
                    for(var i=0;i<data.length;i++){
                        for(var j=0;j<arr.length;j++){
                            if(arr[j]['c_id']==data[i]['c_id']){
                                arr.splice(j,1) ;
                                break;
                            }
                        }
                    ...
                    _this.resultAll=arr;
                    var oArr = Object.keys(_this.resultAll);
                    var le=oArr.length;
        ...
        methods:{
            ...
            choose:function (e) {
                ...
                $.post('student/check_test',{c_id:this.c_id},function (res){
                    if(res=='null') {
                        alert('没有考试权限请与任课教师确认');
                        $('#myModal1').modal('hide');
                    }
                })
            },
            confirm:function () {
                $.post('student/score_add',{c_id:this.c_id},function (res) {
                    if(res==1){
                        alert('考试开始');
                        $('#myModal1').modal('hide');
                        location='student/testing'
                    }else {
                        alert('请重试');
                        $('#myModal1').modal('hide');
                    }
